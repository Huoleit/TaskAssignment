...
class TSP:
    def __init__(self) -> None:
        ...
        self._transit_callback_index = None

    def compute_euclidean_distance_matrix(self, locations, start_ind):
        distances = {}
        for from_counter, from_node in enumerate(locations):
            distances[from_counter] = {}
            for to_counter, to_node in enumerate(locations):
                if from_counter == to_counter or to_counter == start_ind:
    ...
    def solve(self, locations, start_ind):
        self._distance_matrix = self.compute_euclidean_distance_matrix(locations, start_ind)
        self._manager = pywrapcp.RoutingIndexManager(len(locations), 1, start_ind)
        ...
        route = [self._manager.IndexToNode(index), ]
        while not self._routing.IsEnd(index):
            index = solution.Value(self._routing.NextVar(index))
            route.append(self._manager.IndexToNode(index))
        return route

        # if solution:
        #     self.print_solution(self._manager, self._routing, solution)
